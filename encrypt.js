...
let handler = async (m, { conn, text }) => {
	...
	try {
		m.reply(
			"Tunggu sebentar, sedang mengompresi dan mengenkripsi file JavaScript...",
		);
		...
		for (const file of files) {
			...
			if (error) {
				console.error(
					`Terjadi kesalahan dalam mengompresi file ${file}:`,
					error,
				);
				continue;
			}
		...
		if (encryptedFiles.length === 0) {
			m.reply(
				"Tidak ada file JavaScript yang dapat dikompresi dan dienkripsi.",
			);
			return;
		}
		...
		console.error(error);
		m.reply(
			"Terjadi kesalahan dalam mengompresi, mengenkripsi, dan mengirim file zip",
		);
	}
};
