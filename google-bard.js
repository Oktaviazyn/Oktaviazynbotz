...
async function GoogleBard(query) {
	const COOKIE_KEY =
		"awhDhy-7HHtxxRztpGSA13d3-DxQUe_b_mtNK4qzwkdnP85eNsq5RPSY5lvXLn8Wm7gKww.";
	...
	const [snlM0e, blValue] = [
		bardText.match(/"SNlM0e":"(.*?)"/)?.[1],
		bardText.match(/"cfb2h":"(.*?)"/)?.[1],
	];

	const bodyData = `f.req=[null,"[[\\"${encodeURIComponent(
		query,
	)}\\"],null,[\\"\\",\\"\\",\\"\\"]]\"]&at=${snlM0e}`;
...
async function GoogleBardApi(query) {
	...
	const bardRes = await fetch(
		`https://api.azz.biz.id/api/bard?q=${query}&key=global`,
		{ method: "get", headers },
	);
