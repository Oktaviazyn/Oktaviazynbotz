...
const effects = ["0", "1", "2"];
const answer = {
	bad: ["Ya Karna Fotonya Buriq", "Fotonya Buriq Gimana Hasil Mau Bagus"],
	good: ["Thanks", "Terima Kasih"],
};
let handler = async (m, { conn, args, usedPrefix, command }) => {
	...
	if (m.sender in conn.differentMe) throw "_Silahkan Tunggu Sampai Selesai_";
	const q = m.quoted ? m.quoted : m;
	const mime = (q.msg || q).mimetype || q.mediaType || "";
	if (!mime) return m.reply(`Fotonya?`);
...
handler.help = [
	"turnmeanime *ʀᴇᴘʟʏ ꜰᴏᴛᴏ*",
	"drawme *ʀᴇᴘʟʏ ꜰᴏᴛᴏ*",
	"animeface *ʀᴇᴘʟʏ ꜰᴏᴛᴏ*",
];
handler.tags = [tanime];
handler.command = ["turnmeanime", "drawme", "animeface"];
