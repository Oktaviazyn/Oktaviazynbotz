let handler = async (m, { conn, text, usedPrefix, command, participants }) => {
	...
	switch (text) {
		...
		case "enable":
			...
			if (cek)
				return conn.reply(m.chat, `*reminder telah aktif pada grup ini.*`, m);
			await conn.sendPresenceUpdate("composing", m.chat);
		...
		case "disable":
			...
			if (!ce)
				return conn.reply(m.chat, `*reminder belum aktif pada grup ini.*`, m);
			await conn.sendPresenceUpdate("composing", m.chat);
...
handler.help = ["reminder"].map((v) => v + " *ᴏɴ / ᴏꜰꜰ*");
handler.tags = [tgroup];
