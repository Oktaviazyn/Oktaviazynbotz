let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
	if (args.length < 3) {
		return conn.reply(
			m.chat,
			`Contoh: *${usedPrefix}transfer Eris 100 @tag*\n\n*List Yang Bisa Di Transfer :*\n• eris\n• Tabungan\n• Potion\n• trash\n• Diamond\n• Common\n• Uncommon\n• Mythic\n• Legendary\n• String\n🪵 wood\n• rock\n• Iron`.trim(),
			m,
		);
	} else
		try {
			...
			switch (type) {
				case "eris":
					if (global.db.data.users[m.sender].eris >= count * 1) {
						try {
							...
							conn.reply(
								m.chat,
								`Berhasil Mentransfer Eris Sebesar ${count}`.trim(),
								m,
							);
						} catch (e) {
							global.db.data.users[m.sender].eris += count * 1;
						...
						conn.reply(
							m.chat,
							`Eris Kamu Tidak Mencukupi Untuk Mentransfer Sebesar ${count}`.trim(),
							m,
						);
					break;
				case "tabungan":
					if (global.db.data.users[m.sender].atm >= count * 1) {
						...
						conn.reply(
							m.chat,
							`Tidak Mencukupi Untuk mentransfer Uang Dari Bank Sebesar ${count}`.trim(),
							m,
						);
					break;
				case "limit":
					if (global.db.data.users[m.sender].limit >= count * 1) {
						...
						conn.reply(
							m.chat,
							`Limit Kamu Tidak Mencukupi Untuk Mentransfer Sebesar ${count} Limit`.trim(),
							m,
						);
					break;
				case "potion":
				...
				case "trash":
					if (global.db.data.users[m.sender].trash >= count * 1) {
						try {
							...
							conn.reply(
								m.chat,
								`Berhasil Mentransfer ${count} trash`.trim(),
								m,
							);
						} catch (e) {
							global.db.data.users[m.sender].trash += count * 1;
				...
				case "legendary":
					if (global.db.data.users[m.sender].legendary >= count * 1) {
						...
						conn.reply(m.chat, `Legendary Crate Kamu Tiidak Cukup`.trim(), m);
					break;
				case "string":
					if (global.db.data.users[m.sender].string >= count * 1) {
						...
						conn.reply(
							m.chat,
							`String Kamu Tidak Mencukupi Untuk Mentransfer String Sebesar ${count}`.trim(),
							m,
						);
					break;
				case "rock":
					if (global.db.data.users[m.sender].rock >= count * 1) {
						try {
							...
							conn.reply(
								m.chat,
								`Berhasil Mentransfer rock Sebesar ${count}`.trim(),
								m,
							);
						} catch (e) {
							global.db.data.users[m.sender].rock += count * 1;
						...
						conn.reply(
							m.chat,
							`rock Kamu Tidak Mencukupi Untuk Mentransfer rock Sebesar ${count}`.trim(),
							m,
						);
					break;
				case "wood":
					if (global.db.data.users[m.sender].wood >= count * 1) {
						try {
							...
							conn.reply(
								m.chat,
								`Berhasil Mentransfer wood Sebesar ${count}`.trim(),
								m,
							);
						} catch (e) {
							global.db.data.users[m.sender].wood += count * 1;
						...
						conn.reply(
							m.chat,
							`wood Kamu Tidak Mencukupi Untuk Mentransfer wood Sebesar ${count}`.trim(),
							m,
						);
					break;
				case "iron":
					if (global.db.data.users[m.sender].iron >= count * 1) {
						...
						conn.reply(
							m.chat,
							`Iron Kamu Tidak Mencukupi Untuk Mentransfer Iron Sebesar ${count}`.trim(),
							m,
						);
					break;
				default:
