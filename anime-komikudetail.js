...
let handler = async (m, { conn, text }) => {
	...
	try {
		let res = await fetch(
			`https://kiicodeofficial.my.id/api/anime/komiku-detail?url=${text}&apikey=Ceri`,
		);
		let json = await res.json();
		let { awal, terbaru, konsep_cerita, komikus, jumlah_pembaca, end } =
			json.data.metadata;
		res = json.data.chapters.map(
			(v) => `*Chapter:* ${v.chapter}\n*Url:* ${v.url}`,
		).join`\n\n°°°°°°°°°°°°°°°°°°°°°°°°°°°°°\n\n`;
