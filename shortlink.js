...
let handler = async (m, { conn, usedPrefix, args }) => {
	...
	if (args[1] == "tinyurl") {
		let tiny = await (
			await fetch(
				`https://api.lolhuman.xyz/api/shortlink?apikey=02c9d517fcb020c279040776&url=${args[0]}`,
			)
		).json();
	...
	if (args[1] == "ouo") {
		let ouo = await (
			await fetch(
				`https://api.lolhuman.xyz/api/ouoshortlink?apikey=02c9d517fcb020c279040776&url=${args[0]}`,
			)
		).json();
