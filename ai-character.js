...
export async function before(m) {
	...
	if (m.isBaileys || !global.db.data.chats[m.chat].characterai || !m.text)
		return false;
	...
	if (
		words.length === 3 &&
		words[0].toLowerCase() === "characterai" &&
		!isNaN(words[2])
	) {
		...
		if (characterNumber >= 0 && characterNumber < characterNames.length) {
			...
			await this.reply(
				m.chat,
				`*Nama karakter diubah menjadi: ${this.characterai.name}*`,
				m,
			);
		} else {
	...
	if (this.characterai.name) {
		...
		const url = `https://api.yanzbotz.my.id/api/ai/characterai?text=${encodeURIComponent(
			text,
