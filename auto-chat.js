	...
	if (text) {
		try {
			...
			if (text.startsWith("settype")) {
				...
				if (
					this.autochat.settype === 1 &&
					selectedModelIndex < replicateModel.length
				) {
					this.autochat.modelId = replicateModel[selectedModelIndex];
					await this.reply(
						m.chat,
						`Set model to Replicate model ${selectedModelIndex + 1}.`,
						m,
					);
				} else if (
					this.autochat.settype === 2 &&
					selectedModelIndex < cohereModel.length
				) {
					this.autochat.modelName = cohereModel[selectedModelIndex];
					await this.reply(
						m.chat,
						`Set model to Cohere model ${selectedModelIndex + 1}.`,
						m,
					);
				} else {
					await this.reply(
						m.chat,
						"Invalid setmodel value or model type mismatch.",
						m,
					);
				}
			} else if (text.includes("ai") || text.includes("autochat")) {
				const startIdx =
					Math.max(text.indexOf("ai"), text.indexOf("autochat")) + 2;
				...
				if (this.autochat.settype === 1) {
					if (!this.autochat.modelId) {
						await this.reply(
							m.chat,
							"Please set the model ID for Replicate API using setmodel.",
							m,
						);
						return;
					}
					generatedText = await textGenerationApi.generateTextFromReplicate(
						this.autochat.modelId,
						messages,
					);
					...
					if (!this.autochat.modelName) {
						await this.reply(
							m.chat,
							"Please set the model name for Cohere API using setmodel.",
							m,
						);
						return;
					}
					generatedText = await textGenerationApi.generateTextFromCohere(
						this.autochat.modelName,
						messages,
					);
				} else {
...
const cohereModel = ["command-light-nightly", "command-nightly"];
